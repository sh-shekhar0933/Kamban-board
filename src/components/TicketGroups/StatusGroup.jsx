...
const StatusGroup = ({ tickets, order }) => {
  // 1. Group tickets by user ID (using a Map)
  const groupedTickets = new Map();
  if (tickets) {
    tickets.forEach((ticket) => {
      const status = ticket.status;
      if (!groupedTickets.has(status)) {
        groupedTickets.set(status, []);
      }
      groupedTickets.get(status).push(ticket);
    });
  }

  if(!groupedTickets.has("Done")) groupedTickets.set("Done",[]);
  if(!groupedTickets.has("Cancelled")) groupedTickets.set("Cancelled",[]);
  if(!groupedTickets.has("In progress")) groupedTickets.set("In progress",[]);
  if(!groupedTickets.has("Backlog")) groupedTickets.set("Backlog",[]);
  if(!groupedTickets.has("Todo")) groupedTickets.set("Todo",[]);
  ...
  return (
    <div className='container'>
      ...
      {Array.from(groupedTickets.entries()).map(([status, statusTickets]) => {
        // Sort tickets within the user group
        statusTickets.sort(sortTickets);

        // Render user name and ticket cards
        return (
          <div className="cardwidth" key={status}>
            <h2><div>{status==="In progress"&&<svg className='s-icon' width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
...
{status==="Backlog"&&<svg className='s-icon' width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
...
{status==="Todo"&&<svg className='s-icon' width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
...
{status==="Cancelled"&&<svg className='s-icon' width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
...
{status==="Done"&&<svg className='s-icon' width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<path d="M10.951 4.24896C11.283 4.58091 11.283 5.11909 10.951 5.45104L5.95104 10.451C5.61909 10.783 5.0809 10.783 4.74896 10.451L2.74896 8.45104C2.41701 8.11909 2.41701 7.5809 2.74896 7.24896C3.0809 6.91701 3.61909 6.91701 3.95104 7.24896L5.35 8.64792L9.74896 4.24896C10.0809 3.91701 10.6191 3.91701 10.951 4.24896Z" fill="#FBFBFB"/>
  ...
  {status} <span className='length'>{statusTickets.length}</span></div>{statusTickets.length!==0&&<div><svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<path d="M3 6.5C3.39782 6.5 3.77936 6.65804 4.06066 6.93934C4.34196 7.22064 4.5 7.60218 4.5 8C4.5 8.39782 4.34196 8.77936 4.06066 9.06066C3.77936 9.34196 3.39782 9.5 3 9.5C2.60218 9.5 2.22064 9.34196 1.93934 9.06066C1.65804 8.77936 1.5 8.39782 1.5 8C1.5 7.60218 1.65804 7.22064 1.93934 6.93934C2.22064 6.65804 2.60218 6.5 3 6.5ZM8 6.5C8.39782 6.5 8.77936 6.65804 9.06066 6.93934C9.34196 7.22064 9.5 7.60218 9.5 8C9.5 8.39782 9.34196 8.77936 9.06066 9.06066C8.77936 9.34196 8.39782 9.5 8 9.5C7.60218 9.5 7.22064 9.34196 6.93934 9.06066C6.65804 8.77936 6.5 8.39782 6.5 8C6.5 7.60218 6.65804 7.22064 6.93934 6.93934C7.22064 6.65804 7.60218 6.5 8 6.5ZM13 6.5C13.3978 6.5 13.7794 6.65804 14.0607 6.93934C14.342 7.22064 14.5 7.60218 14.5 8C14.5 8.39782 14.342 8.77936 14.0607 9.06066C13.7794 9.34196 13.3978 9.5 13 9.5C12.6022 9.5 12.2206 9.34196 11.9393 9.06066C11.658 8.77936 11.5 8.39782 11.5 8C11.5 7.60218 11.658 7.22064 11.9393 6.93934C12.2206 6.65804 12.6022 6.5 13 6.5Z" fill="#5C5C5E"/>
            ...
            {statusTickets.map((ticket) => (
              <TicketCard key={ticket.id} ticket={ticket} group="status" />
            ))}
          </div>
        );
      })}
