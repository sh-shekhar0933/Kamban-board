...
const PriorityGroup = ({ tickets, order }) => {
  // 1. Group tickets by user ID (using a Map)
  const groupedTickets = new Map();
  if (tickets) {
    tickets.forEach((ticket) => {
      const priority = ticket.priority;
      if (!groupedTickets.has(priority)) {
        groupedTickets.set(priority, []);
      }
      groupedTickets.get(priority).push(ticket);
    });
  }

  // Sort the grouped tickets in-place by priority in descending order
  for (const [priority, tickets] of groupedTickets) {
    tickets.sort((a, b) => b.priority - a.priority); // Descending priority
  }

  const ord = [0, 4, 3, 2, 1];

  // 2. Render user groups and tickets
  return (
    <div className='container'>
      
      <div className='type-list'>
      {ord.map((prio) => {
        const requiredTickets = groupedTickets.get(prio);

        // Render priority heading and tickets (if any)
        return (
          requiredTickets && (
            <div className="cardwidth" key={prio}>
              <h2> <div>{prio===0&& <span><svg className='prio' width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
...
No Priority</span>} {prio===1&& <span><svg className='prio' width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
...
Low</span>} {prio===2&& <span><svg className='prio' width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M3.5 8H2.5C1.94772 8 1.5 8.44772 1.5 9V13C1.5 13.5523 1.94772 14 2.5 14H3.5C4.05228 14 4.5 13.5523 4.5 13V9C4.5 8.44772 4.05228 8 3.5 8Z" fill="#5C5C5E"/>
...
{prio===3&& <span><svg className='prio' width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M3.5 8H2.5C1.94772 8 1.5 8.44772 1.5 9V13C1.5 13.5523 1.94772 14 2.5 14H3.5C4.05228 14 4.5 13.5523 4.5 13V9C4.5 8.44772 4.05228 8 3.5 8Z" fill="#5C5C5E"/>
<path d="M8.5 5H7.5C6.94772 5 6.5 5.44772 6.5 6V13C6.5 13.5523 6.94772 14 7.5 14H8.5C9.05228 14 9.5 13.5523 9.5 13V6C9.5 5.44772 9.05228 5 8.5 5Z" fill="#5C5C5E"/>
...
High</span>}
{prio===4&& <span><svg className='prio' width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
...
Urgent</span>}
 ...
 <span className='length'>{requiredTickets.length}</span></div><div><svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<path d="M3 6.5C3.39782 6.5 3.77936 6.65804 4.06066 6.93934C4.34196 7.22064 4.5 7.60218 4.5 8C4.5 8.39782 4.34196 8.77936 4.06066 9.06066C3.77936 9.34196 3.39782 9.5 3 9.5C2.60218 9.5 2.22064 9.34196 1.93934 9.06066C1.65804 8.77936 1.5 8.39782 1.5 8C1.5 7.60218 1.65804 7.22064 1.93934 6.93934C2.22064 6.65804 2.60218 6.5 3 6.5ZM8 6.5C8.39782 6.5 8.77936 6.65804 9.06066 6.93934C9.34196 7.22064 9.5 7.60218 9.5 8C9.5 8.39782 9.34196 8.77936 9.06066 9.06066C8.77936 9.34196 8.39782 9.5 8 9.5C7.60218 9.5 7.22064 9.34196 6.93934 9.06066C6.65804 8.77936 6.5 8.39782 6.5 8C6.5 7.60218 6.65804 7.22064 6.93934 6.93934C7.22064 6.65804 7.60218 6.5 8 6.5ZM13 6.5C13.3978 6.5 13.7794 6.65804 14.0607 6.93934C14.342 7.22064 14.5 7.60218 14.5 8C14.5 8.39782 14.342 8.77936 14.0607 9.06066C13.7794 9.34196 13.3978 9.5 13 9.5C12.6022 9.5 12.2206 9.34196 11.9393 9.06066C11.658 8.77936 11.5 8.39782 11.5 8C11.5 7.60218 11.658 7.22064 11.9393 6.93934C12.2206 6.65804 12.6022 6.5 13 6.5Z" fill="#5C5C5E"/>
              ...
              {requiredTickets.map((ticket) => (
                <TicketCard key={ticket.id} ticket={ticket} group="priority" />
              ))}
            </div>
          )
        );
